...
class ElectronBuilder {
    ...
    createInstaller(opts,callback){
        ...
        if(isMac){
            ...
            installer({
                src: opts.appPath,
                dest: '/Users/mxj/zoom/mtool/dev-release/dist',



                "icon": "/Users/mxj/zoom/mtool/build/icon.ico",
                "tags": [
                    "Utility"
                ],
                exe: opts.appPath + '/mToolwin.exe',

            }).then((res)=>{
                debugger
                //console.log(`Successfully created package at ${options.dest}`)

                callback && callback(res.exe);
            }).catch((err)=>{
...
module.exports = electronBuilderCase;
...
electronBuilderCase.pack((appPath)=>{
    electronBuilderCase.createInstaller({
        ...
        electronBuilderCase.publish({
            localFile: installerPath,
            Key: 'dev-releases/win/mxj_desktop_app_wrapper-5.9.15-setup.exe'
        })
