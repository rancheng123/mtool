...
function sdkauthCB(status) {
  ...
  if (ZoomAuthResult.AUTHRET_SUCCESS == status){
    ...
    zoomdirectshare = zoomauth.GetDirectShare();
      ...
      //showLoginWindow();
      //直接登录 （此处的账号 安全需要考虑 ？？？？？？？？）
      var username = 'lihai@mydreamplus.com';
      var psw  = 'Abcd1234'
      console.log('登录')
      functionObj.login(username,psw)
...
function ProcSDKReady(){
    ...
    //直接认证
    var appkey = 'IhzfMgizOJVvh4Q2HoRyCCdu32iGwtleFI3m';
    var appsecret = 'wkZ2hpcBY5i9a1uN63tmoFtMfIuCQJAoeSHA';

    console.log('认证')
    functionObj.sdkauth(appkey, appsecret);
}
...
let functionObj = {
  ...
  setDomain: function(domain, enable_log) {
    const opts = {
      ...
      langid: ZoomSDK_LANGUAGE_ID.LANGUAGE_Chinese_Simplified
    }
    debugger
    if (platform == 'win32') {
...
global.functionObj = functionObj;
...
var domain = 'https://www.zoomus.cn'
//设置域名
console.log('设置域名')
functionObj.setDomain(domain,true)
